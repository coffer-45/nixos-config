...
in
{
  ...
  # A malformed KDL file must fail Home Manager activation rather than leave
  # the next Niri session without working key bindings.
  home.activation.validateNiriConfig = lib.hm.dag.entryBefore [ "writeBoundary" ] ''
    $DRY_RUN_CMD ${pkgs.niri}/bin/niri validate --config ${niriConfig}
  '';
}
